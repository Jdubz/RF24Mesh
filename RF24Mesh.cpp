...
bool RF24Mesh::begin(uint8_t channel, rf24_datarate_e data_rate, uint32_t timeout){
  ...
  Serial.println("call radio begin");
  radio.begin();
  radio_channel = channel;
  Serial.println("call radio setChannel");
  radio.setChannel(radio_channel);
  Serial.println("call radio setDataRate");
  radio.setDataRate(data_rate);
  network.returnSysMsgs = 1;

  if(getNodeID()){ //Not master node
    Serial.println("not master node");
    mesh_address = MESH_DEFAULT_ADDRESS;
    if(!renewAddress(timeout)){
      Serial.println("not renewAddress: return 0");
      return 0;
    }
  }else{
  ...
  Serial.println("mesh.begin returning 1");
  return 1;
}
